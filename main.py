...
#print(pd.DataFrame(Counter(spam_corpus).most_common(20)))
#print(pd.DataFrame(Counter(ham_corpus).most_common(20)))
...
p_prediction = model.predict(x_test)
p_prediction = p_prediction.flatten()
y_prediction = np.where(p_prediction >= 0.5, 1, 0)
